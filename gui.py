...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute(
    "CREATE TABLE IF NOT EXISTS 'INVENTORY' (SNO INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,prodName TEXT, prodCode "
    "TEXT, prodWeig TEXT, prodQty TEXT, prodDate TEXT, prodDispatch TEXT, Delivery TEXT, Dispatch TEXT, Category"
    " INTEGER, Fragile INTEGER, FastShip INTEGER, Supervisor TEXT)")
    cursor.execute(
    "CREATE TABLE IF NOT EXISTS 'FRAGILE' (SNO INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,prodName TEXT, prodCode "
    "TEXT, prodWeig TEXT, prodQty TEXT, prodDate TEXT, prodDispatch TEXT, Delivery TEXT, Dispatch TEXT, Category"
    " INTEGER, Supervisor TEXT)")
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS 'FAST' (SNO INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,prodName TEXT, prodCode "
        "TEXT, prodWeig TEXT, prodQty TEXT, prodDate TEXT, prodDispatch TEXT, Delivery TEXT, Dispatch TEXT, Category"
        " INTEGER, Supervisor TEXT)")
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS 'NONE' (SNO INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,prodName TEXT, prodCode "
        "TEXT, prodWeig TEXT, prodQty TEXT, prodDate TEXT, prodDispatch TEXT, Delivery TEXT, Dispatch TEXT, Category"
        " INTEGER, Supervisor TEXT)")
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS 'BOTH' (SNO INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,prodName TEXT, prodCode "
        "TEXT, prodWeig TEXT, prodQty TEXT, prodDate TEXT, prodDispatch TEXT, Delivery TEXT, Dispatch TEXT, Category"
        " INTEGER, Supervisor TEXT)")
    conn.commit()
...
def DatabaseAdd():
    Database()
    global conn, cursor
    cursor.execute("INSERT INTO 'INVENTORY'(prodName, prodCode, prodWeig, prodQty, prodDate, prodDispatch, Delivery,"
                   " Dispatch, Category, Fragile, FastShip, Supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   (str(v1.get()), str(v2.get()), str(v3.get()), str(v4.get()), str(v5.get()), str(v6.get()),
                    str(v7.get()), str(v8.get()), str(v.get()), str(var.get()), str(var1.get()), str(log.get())))

    if var.get() == 1 and var1.get() == 1:
        cursor.execute(
            "INSERT INTO 'BOTH'(prodName, prodCode, prodWeig, prodQty, prodDate, prodDispatch, Delivery,"
            " Dispatch, Category, Supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (str(v1.get()), str(v2.get()), str(v3.get()), str(v4.get()), str(v5.get()), str(v6.get()),
             str(v7.get()), str(v8.get()), str(v.get()), str(log.get())))
        conn.commit()
        v1.set(""), v2.set(""), v3.set(""), v4.set(""), v5.set(""), v6.set(""), v7.set(""), v8.set(""), \
        v.set("Appliances"), log.set("")

    elif var.get() == 0 and var1.get() == 1:
        cursor.execute(
            "INSERT INTO 'FAST'(prodName, prodCode, prodWeig, prodQty, prodDate, prodDispatch, Delivery,"
            " Dispatch, Category, Supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (str(v1.get()), str(v2.get()), str(v3.get()), str(v4.get()), str(v5.get()), str(v6.get()),
             str(v7.get()), str(v8.get()), str(v.get()), str(log.get())))
        conn.commit()
        ...
        cursor.execute(
            "INSERT INTO 'FRAGILE'(prodName, prodCode, prodWeig, prodQty, prodDate, prodDispatch, Delivery,"
            " Dispatch, Category, Supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (str(v1.get()), str(v2.get()), str(v3.get()), str(v4.get()), str(v5.get()), str(v6.get()),
             str(v7.get()), str(v8.get()), str(v.get()), str(log.get())))
        conn.commit()
    ...
    elif var.get() == 0 and var1.get() == 0:
        cursor.execute(
            "INSERT INTO 'NONE'(prodName, prodCode, prodWeig, prodQty, prodDate, prodDispatch, Delivery,"
            " Dispatch, Category, Supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (str(v1.get()), str(v2.get()), str(v3.get()), str(v4.get()), str(v5.get()), str(v6.get()),
             str(v7.get()), str(v8.get()), str(v.get()), str(log.get())))
        conn.commit()
    ...
    conn.commit()
    v1.set(""), v2.set(""), v3.set(""), v4.set(""), v5.set(""), v6.set(""), v7.set(""), v8.set(""), \
    v.set("Appliances"), var.set("0"), var1.set("0"), log.set("")
...
def Exit():
    result = tm.askquestion('Inventory Management', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        cursor.close()
        conn.close()
...
def Chnglog():
    tm.showinfo("Changelog", "1 - GUI \n2- SQLite3 Database integration")


def About():
    tm.showinfo("About", "DBMS Project\nMade by Arush Sharma")
root = Tk()
